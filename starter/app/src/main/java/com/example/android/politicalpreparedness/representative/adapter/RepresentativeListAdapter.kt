...
class RepresentativeViewHolder(val binding: RepresentativeItemBinding) : RecyclerView.ViewHolder(binding.root) {
    ...
    fun bind(item: Representative) {
        ...
        item.official.urls?.let { showWebLinks(it) }
    ...
    private fun showSocialLinks(channels: List<Channel>) {
        ...
        if (!facebookUrl.isNullOrBlank()) {
            showLink(binding.facebookIcon, facebookUrl)
        }

        val twitterUrl = getTwitterUrl(channels)
        if (!twitterUrl.isNullOrBlank()) {
            showLink(binding.twitterIcon, twitterUrl)
        }
    }

    private fun showWebLinks(urls: List<String>) {
        showLink(binding.webIcon, urls.first())
    }
    ...
    private fun showLink(view: ImageView, url: String) {
